...
# Adapted from original (see license and credits below) by Nick van der Meijs
# convenience functions for accessing waveforms
...
VERSIONSTRING = "LTSPy 1.04-3 (Torsten Lehmann), update 2 (Nick van der Meijs)"
...
class SimData(object):
    ...
    dat.v ("name", index = 0, pvals = None) is convenience function 
        returning values for a trace directly,
        index is the step number (if stepped)
        if stepped: equivalent to dat.values[dat.variables.index(name)][index]
        else: equivalent to dat.values[dat.variables.index(name)]
        dat.getvalues("time") returns first trace (index 0)

    dat.steppvals can be used to cycle through traces based on stepped parameter
        can be used as follows:
        for c, vgs in enumerate (data.stepvals ("vgs")):
            for l in data.stepvals ("l"):
                vds = data.v("V(out)", pvals = [["vgs", "l"], [vgs, l]])
                idd = data.v("Id(M1)", pvals = [["vgs", "l"], [vgs, l]])
    '''
    ...
    def __init__(self,filename,tranfix=True,variablelist=None):
        ...
        if self.nosteps != None and self.nosteps != 0:#'and self.nosteps!=0 - fix for stepped .op
            ...
            if not os.path.isfile(logfilename):
                print(f'Warning: no logfile for step info (file "{logfilename}" not found)')
                self.stepvariables = []
                self.stepvalues = []
    ...
    def stepvals (self, param):
        """ return a list of values for stepped parameter 'param'"""
        pos = self.ppos (param)
        vals = []
        for v in self.stepvalues[pos]:
            if v not in vals:
                vals.append (v)
        return vals

    def ppos (self, param):
        """ get index of parameter 'param' (string)"""
        try:
            pos = self.stepvariables.index (param)
        except:
            raise ValueError ( "param '%s' not found" % param)
        return pos


    def v (self, name, index = 0, pvals = None):
        if name == "time":
            ix = 0
        else:
            try:
                ix = self.variables.index(name)
            except:
                raise ValueError ( "trace '%s' not found" % name)
        ...
        if pvals is None:
            return (self.values[ix][index])

        # find the index by matching values in pvals.
        # pvals looks like [["par1", "par2"], [val1, val2]]

        # search will be a list in the right order for values to be matched.
        search = [None] * len (self.stepvariables)
        for i, param in enumerate (pvals[0]):
            pos = self.ppos (param)
            search [pos] = pvals[1][i]

        # now, find the index, by comparing values
        for index in range (self.nosteps):
            # print ("index", index, "search", search, "param", self.stepvalues[0][index], self.stepvalues[1][index])
            match = 0
            for j in range (len (self.stepvariables)):
                # print ("compare",  search[j], self.stepvalues [j][index])
                if search[j] == self.stepvalues [j][index]:
                   match += 1
            if match == len (self.stepvariables):
                return (self.values[ix][index])


        print ("pvals not found", pvals)
        return -1


    def summary (self):
        summary  = VERSIONSTRING + '\n'
        summary += f"LTSpice Version: {self.command.decode ()}\n"
        summary += f"File: {self.filename}\n"
        summary += f"Title: {self.title.decode ()}\n"
        summary += f"Date: {self.date.decode ()}\n"
        summary += f"Plotname: {self.plotname.decode ()}\n"
        summary += f"No. variables: {self.novariables}\n"
        summary += f"Variables: {[v.decode() for v in self.variables]}\n"
        summary += f"Variables Types: {[t.decode () for t in self.variabletypes]}\n"
        summary += f"No. points: {self.nopoints}\n"
        summary += f"Offset: {self.offset}\n"
        summary += f"Output: {self.output}\n"
        summary += f"Binary: {self.binary}\n"
        summary += f"Analysis: {self.analysis.decode ()}\n"
        summary += f"Flags: {[f.decode () for f in self.flags]}\n"

        if self.stepped:
            summary += f"Steplengths: {self.steplen}\n"
            summary += f"Number of steps: {self.nosteps}\n"
            if not len (self.stepvariables):
                summary += f"Step variables and step values not known (.log file missing?)\n"
            else:
                summary += f"Step variables: {[v.decode () for v in self.stepvariables]}\n"
                summary += f"Step values: {self.stepvalues}\n"
        return summary
